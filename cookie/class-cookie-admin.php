	...
	class cmplz_cookie_admin {
	    ...
	    */
		...
		public function sync_progress() {
			...
			if ( ! function_exists( 'curl_version' ) ) {
				$disabled    = "disabled";
				$explanation = cmplz_notice( __( 'Your server does not have CURL installed, which is required for the sync. Please contact your hosting company to install CURL.',
					'complianz-gdpr' ), 'warning', false, false );
			}

			if ( $data_cookies['count'] == 0 && $data_services['count'] == 0 ) {
				$disabled    = "disabled";
				$explanation = cmplz_notice( __( 'Synchronization disabled: This happens when all cookies have synchronized to cookiedatabase.org in the last week.',
					'complianz-gdpr' ), 'warning', false, false );
			}
