		...
		if (complianz.tm_categories) {
			for (var i = 0; i < complianz.cat_num; i++) {
				if ($('.cmplz_' + i).is(":checked")) {
					cmplzSetCookie(cmplzPrefix+'event_' + i, 'allow', complianz.cookie_expiry);
					cmplz_wp_set_consent(cmplzPrefix+'event_' + i, 'allow');
				} else {
					cmplzSetCookie(cmplzPrefix+'event_' + i, 'deny', complianz.cookie_expiry);
					cmplz_wp_set_consent(cmplzPrefix+'event_' + i, 'deny');
				}
			}
		}
